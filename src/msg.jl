"""
    tracymsg(msg::AbstractString; color::Union{Number,Symbol,NTuple{3, Integer}, Nothing}=nothing)
...
"""
function tracymsg(msg::AbstractString; color::Union{Number,Integer,Symbol,NTuple{3,Integer},Nothing}=nothing)
    if color === nothing
        @ccall libtracy.___tracy_emit_message(msg::Cstring, length(msg)::Csize_t)::Cvoid
    else
        @ccall libtracy.___tracy_emit_messageC(msg::Cstring, length(msg)::Csize_t, _tracycolor(color)::UInt32)::Cvoid
    end
end
